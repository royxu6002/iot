~(function(win){
	...
	ImgUploadeFiles.prototype = {
		...
		ready : function(){
			...
			this.imgcontent.innerHTML = 'Please upload maximum <b style="color:red">'+this.MAX+'</b> less than'+_self.MW+' * '+_self.MH+' piexl images';
	...
	var _private = {
		startUploadImg : function(o,files,MAX,callback,W,H){
			...
			if(files.length > MAX ){
				alert('shoudd less than or equal to'+MAX+'images');
				return false;
			};
			var lens = $(o).find('li').length ;
			if(lens >= MAX ){
				alert('should less than or equal tos'+MAX+'images');
				return false;
			};
